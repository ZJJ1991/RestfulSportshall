...
class Engine(object):
    ...
    def create_sports_table(self):
        ...
        stmnt = 'CREATE TABLE sports(sport_id INTEGER PRIMARY KEY AUTOINCREMENT, \
                    sportname TEXT UNIQUE, hallnumber INTEGER, time TEXT, \
                    note TEXT,'
        con = sqlite3.connect(self.db_path)
        with con:
	...
	def create_sportorder_table(self):
        '''
        ...
        '''
		keys_on = 'PRAGMA foreign_keys = ON'
        stmnt = 'CREATE TABLE order(order_id INTEGER PRIMARY KEY AUTOINCREMENT, \
                    sportname TEXT, timestamp INTEGER, hallnumber INTEGER, \
...
class Connection(object):
    ...
    def _create_sport_object(self, row):
		...
		sport_hall = row['hallnumber']
		sport_time = row['time']
		sport_note = row['note']
        sport = {'sport_id': sport_id, 'sport_name': sportname,'hallnumber': sport_hall,'time': sport_time, 'note': sport_note}
        return sport
    ...
    def _create_sport_list_object(self, row):
        ...
        :param row: The row obtained from the database.
        :type row: sqlite3.Row
        :return: a dictionary with the keys 'sport_id', 'sport_name',
		'hallnumber': sport_hall,'time', 'note'
    ...
    def _create_order_object(self, row):
        ...
        order = {'orderid': order_id, 'user_nickname': user_nickname,
                   'timestamp': order_timestamp, 'sport_name': sportname, 'user_id': user_id}
        return order
    ...
    def _create_order_list_object(self, row):
        ...
        :return: a dictionary with the keys ``orderid``, ``usernickname``,
            ``timestamp`` and ``sportname`` and ``user id``.
        ...
        order = {'order_id': order_id, 'user_nickname': user_nickname,
                   'timestamp': order_timestamp, 'sportname': sport_name, 'user_id': user_id}
        return order
    ...
    def get_order(self, order_id):
        ...
        :param orderid: The id of the message. Note that messageid is a
    ...
    def create_order(self, sportname, body, sender="Anonymous",
        ...
        * HOW TO TEST: Use the database_api_tests_message. The following tests
        ...
        _timesviewed = int(0)
        _reply_to = replyto
        _user_nickname = sender
        _editor_nickname = 'Anonymous'
        ...
        if replyto is not None:
            reply_to = 'SELECT * from messages WHERE message_id = ?'
            pvalue = (replyto,)
            cur.execute(reply_to, pvalue)
            messages = cur.fetchall()
            if len(messages) < 1:
                return None  
                    
        query1 = 'INSERT INTO messages(title,body,timestamp,ip,timesviewed,reply_to,user_nickname,user_id) VALUES(?,?,?,?,?,?,?,?)'
        pvalue1 = (_title,_body,_timestamp,_ip,_timesviewed,_reply_to,_user_nickname,_user_id)
        # cur = self.con.cursor()
	...
	def modify_message(self, messageid, title, body, editor="Anonymous"):
        ...
        * Remember to activate the foreign key support
        ...
        _title = title
        _body = body
        _editor_nickname = editor
        self.set_foreign_keys_support()
        self.con.row_factory = sqlite3.Row
        cur = self.con.cursor()
        # pvalue = (messageid,)
        pvalue = (_title,_body,_editor_nickname,messageid)
        cur.execute(query, pvalue)
        self.con.commit()
        if cur.rowcount < 1:
            return None
    ...
    def get_order_time(self, messageid):
        '''
